...
function Items({token}){
  ...
  const [form,setForm]=useState({name:'',price:0,unit:'',cat:'veg'});
  ...
  const save=async()=>{
    ...
    if(res.ok){ 
      setForm({name:'',price:0,unit:'',cat:'veg'}); 
      load(); 
  ...
  return (
    <div>
      ...
      <div style={{display:'flex', gap:8, alignItems:'center', flexWrap:'wrap'}}>
        ...
        <input placeholder="Unit (e.g. 1 kg / 500 g / 1 dozen)" value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})}/>
      ...
      <ul>
        {items.map(i=> 
          <li key={i._id}>
            {i.name} — £{i.price.toFixed(2)} / {i.unit || '-'} — {i.cat} 
...
function Offers({token}){
  ...
  const save=async()=>{
    ...
    if(res.ok){ 
      setForm({title:'',description:'',discountPct:10,active:true}); 
      load(); 
    } else alert('Save failed');
  };
  const remove=async(id)=>{
  ...
  return (
    <div>
      ...
      <ul>
        {offers.map(o=> 
          <li key={o._id}>
            <strong>{o.title}</strong> — {o.discountPct}% 
  ...
  return (
    <div>
      ...
      <ul>
        {orders.map(o=> 
          <li key={o._id}>
            £{o.total?.toFixed?.(2)} — {new Date(o.createdAt).toLocaleString()} — {o.paymentStatus}
          </li>
        )}
      </ul>
    </div>
